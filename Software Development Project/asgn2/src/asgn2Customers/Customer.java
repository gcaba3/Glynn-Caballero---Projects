...
public abstract class Customer {
	...
	public Customer(String name, String mobileNumber, int locationX, int locationY, String type) throws CustomerException{
		...
		if(name.trim().isEmpty()){
			throw new CustomerException("Customer name can't be all spaces or empty");
		}else if (name.length() < minNameLength || name.length() > maxNameLength){
			throw new CustomerException("Customer names can only contain 1 to 20 letters");
		}else {
			this.name = name;
		}
		
		//validate mobileNumber parameters
		final int mobileNumberLength = 10;
		if(mobileNumber.charAt(0) != '0'){
			throw new CustomerException("A mobile number must have a starting 0");
		} else if (mobileNumber.length() != mobileNumberLength){
			throw new CustomerException("A mobile number consists of only 10 numbers");
		} else {
			this.mobileNumber = mobileNumber;
		}
		
		//validate the X and Y locations parameters
		final int maxBlocks = 10;
		if(Math.abs(locationX) > maxBlocks || Math.abs(locationY) > maxBlocks){
			throw new CustomerException("The customers X or Y location can't be greater than 10");
		} else {
